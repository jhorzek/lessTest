test_that("testing mcp-WLS", {
  ...
  for(estimator in c("wls", "uls", "dwls")){
    ...
    lslxParameter <- matrix(NA, 
                            nrow = length(lambdas), 
                            ncol = length(fitLslx$extract_coefficient(lambda = 0, delta = theta)))
    colnames(lslxParameter) <- names(fitLslx$extract_coefficient(lambda = 0, delta = theta))
    ...
    for(l in 1:length(lambdas)){
      ...
      regWls[l] <- fitLslx$extract_numerical_condition(lambda = lambdas[l],
                                                       delta = theta)["objective_value"]
    }
    ...
    rsemGlmnet <- mcp(lavaanModel = modelFit, 
