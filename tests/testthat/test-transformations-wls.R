test_that("testing transformations - wls", {
  library(lavaan)
  library(lessSEM)
  library(OpenMx)
  
  for(estimator in c("wls", "dwls", "uls")){
...
'
    ...
    PoliticalDemocracyExt <- mvtnorm::rmvnorm(n = 5000, 
                                              mean = apply(PoliticalDemocracy, 2, mean), 
                                              sigma = cov(PoliticalDemocracy))
    colnames(PoliticalDemocracyExt) <- colnames(PoliticalDemocracy)
    
    model <- sem(model1, 
                 data = PoliticalDemocracyExt, 
                 meanstructure = TRUE, 
                 do.fit = FALSE,
                 estimator = estimator)
...
'
    ...
    testthat::expect_equal(all(abs((gradients[names(gradientsNoTransformations)] -
                                     gradientsNoTransformations[names(gradientsNoTransformations)])/
                                     gradients[names(gradientsNoTransformations)]) < 1e-6, na.rm = TRUE), 
                           TRUE)
    
    testthat::expect_equal(all(
      abs(
      (Hessian[names(gradientsNoTransformations), names(gradientsNoTransformations)] - 
         HessianNoTransformations[names(gradientsNoTransformations), names(gradientsNoTransformations)])/
        Hessian[names(gradientsNoTransformations), names(gradientsNoTransformations)]) < 1e-5, na.rm = TRUE), TRUE)
  ...
  "
    ...
    modelMx <- mxModel(
      ...
      mxPath(from = manifests, 
             ...
             arrows = 2,
             free = TRUE),
      mxData(observed = PoliticalDemocracyExt, type = "raw"),
      latentVars = latents,
      manifestVars = manifests,
      type = "RAM",
      mxFitFunctionWLS(type = toupper(estimator))
    )
    fitMx <- mxRun(modelMx)
